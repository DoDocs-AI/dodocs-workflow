...
5. **Commit updates to the feature branch**:
   ```bash
   git add <updated-doc-files>
   git commit -m "docs: sync project docs for <feature-slug>"
   ```

6. **Produce DOC-SYNC-REPORT.md** at the path specified by the orchestrator (e.g., `docs/ship/<run-id>/<feature-slug>/DOC-SYNC-REPORT.md`):
