Upgrade dodocs-workflow to the latest release from GitHub.
...
3. Show current version from `~/.opencode/.dodocs-workflow-version`
4. Download and execute install script from the latest tag:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/DoDocs-AI/dodocs-workflow/refs/tags/$LATEST_TAG/install-opencode.sh | bash
   ```
5. Clear update check cache: `rm -f /tmp/dodocs-workflow-update-check-opencode`
